...
import { Phone, Lock, ArrowRight, Loader2, Smartphone } from 'lucide-react';
import { User } from '../../types';
import { db, doc, getDoc } from '../../firebase.ts';
...
const LoginPage: React.FC<LoginPageProps> = ({ onLogin }) => {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      // 1. Fetch user from Firestore by mobile (as document ID)
  ...
  return (
    <div className="min-h-screen bg-white flex flex-col p-8">
      ...
      <form onSubmit={handleLogin} className="flex flex-col gap-5">
        ...
        <div className="flex flex-col gap-2">
          <label className="text-sm font-semibold text-gray-600 ml-1">৪ বা ৬ সংখ্যার পিন</label>
          <div className="relative">
            <input
              type="password"
              required
      ...
      <div className="mt-auto pt-8 text-center">
        <p className="text-gray-500">
          একাউন্ট নেই? <button onClick={() => navigate('/register')} className="text-green-600 font-bold">নতুন একাউন্ট খুলুন</button>
        </p>
      </div>
    </div>
  );
