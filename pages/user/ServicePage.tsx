...
// Fix: Corrected relative path to constants.tsx which is in the same directory.
import { OPERATORS, MOCK_PACKAGES } from './constants.tsx';
import { User, Operator } from '../../types.ts';
import { sendAdminNotification, formatOrderMsg } from '../../services/telegramService.ts';
...
const ServicePage: React.FC<ServicePageProps> = ({ user }) => {
  ...
  const [step, setStep] = useState(1); // Generic step tracker
  ...
  const handleNext = () => {
    // If we are at the final confirmation step, send notification
  ...
  // Recharge Package Filter (Used for Internet/Minute/Bundle)
  const filteredPackages = MOCK_PACKAGES.filter(p => p.operator === selectedOperator);
                ...
                <div className="bg-white p-6 rounded-[32px] shadow-sm border border-gray-100 flex flex-col gap-6">
                  ...
                  <div className="flex flex-col gap-2">
                    ...
                    <div className="flex gap-2 mt-1">
                      {['২০', '৫০', '১০০', '৫০০'].map(amt => (
                        <button 
                          key={amt}
                          onClick={() => setAmount(amt)}
                          className="px-4 py-2 bg-gray-100 hover:bg-green-100 hover:text-green-700 rounded-full text-xs font-bold transition-colors"
                        >
                          ৳{amt}
                        </button>
                      ))}
